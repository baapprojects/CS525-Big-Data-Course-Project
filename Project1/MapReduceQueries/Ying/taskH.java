...
public static class Map extends MapReduceBase implements Mapper<LongWritable, Text, IntWritable, Text>
{
	...
	public void map(LongWritable key, Text value, OutputCollector<IntWritable, Text> output, Reporter reporter)throws IOException
	{
		...
		{
			//Deal with data from friends.txt, get myFriend attribute only
			ID.set(Integer.parseInt(splits[1])); //get myFriend
			output.collect(ID,new Text("F"));
		}
		else
		{
			//Deal with data from accesslog.txt, get accesslog attribute only
			ID.set(Integer.parseInt(splits[2])); //get accesslog
			output.collect(ID,new Text("A"));
		}
	...
	public void reduce(IntWritable key, Iterator<Text> values, OutputCollector<Text, IntWritable> output, Reporter reporter)throws IOException
	{
		...
		while (values.hasNext())
		{
			...
			else
			{
				...
				countToBewhatpage = +countToBewhatpage ;
			}
		}
...
public static void main(String[] args) throws Exception
{
	...
	FileInputFormat.addInputPath(conf, new Path("/ywang/input/accesslog.txt"));
